#!/usr/bin/env python3
"""Run live_topk_bridge.py from a camera CSV without storing credentials in the command line."""
...
def load_cameras(csv_path: Path) -> list[dict[str, str]]:
...
def main() -> int:
    args = parse_args()
    camera_csv = resolve_path(args.camera_csv)
    rows = load_cameras(camera_csv)
    ...
    if args.bridge_args:
        extra = args.bridge_args[1:] if args.bridge_args[0] == "--" else args.bridge_args
