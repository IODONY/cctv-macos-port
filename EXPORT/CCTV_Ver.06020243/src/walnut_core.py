...
class WalnutAnalyzer:
            ...
            people.append(
                {
                    ...
                    "regions": regions,
                    "face_visibility": self._compute_face_visibility(kpts_xy, kpts_conf),
    ...
    def _build_person_view(self, track, person):
        ...
        return {
            ...
            "face_visibility": person.get("face_visibility", {}),
    ...
    def _compute_face_visibility(self, kpts_xy, kpts_conf):
        ...
        right_ear_idx = 4
        visible = {
            "nose": self._kp_valid(kpts_xy, kpts_conf, nose_idx),
            "left_eye": self._kp_valid(kpts_xy, kpts_conf, left_eye_idx),
            "right_eye": self._kp_valid(kpts_xy, kpts_conf, right_eye_idx),
            "left_ear": self._kp_valid(kpts_xy, kpts_conf, left_ear_idx),
            "right_ear": self._kp_valid(kpts_xy, kpts_conf, right_ear_idx),
        }
        return {
            **visible,
            "visible_count": int(sum(1 for value in visible.values() if value)),
            "has_nose": bool(visible["nose"]),
            "has_eyes": bool(visible["left_eye"] and visible["right_eye"]),
        }
